...
def unit(q: jnp.ndarray):
    q = jnp.asarray(q)
    q_norm = norm(q)
    return jnp.where(q_norm < 0.000001, jnp.zeros_like(q), q / q_norm)
...
@jax.jit
def hamilton_product(q: jnp.ndarray, w: jnp.ndarray):
    """(Schaub 3.112)"""
    qw, qx, qy, qz = q
    wx, wy, wz = w
...
@jax.jit
def angle_axis_to_q(angle: float, axis: jnp.ndarray, degrees=False):

    angle_rad = jnp.where(degrees, angle * DEG_TO_RAD, angle)
    unit_axis = unit(axis)

    w = jnp.cos(angle_rad / 2)
    w = jnp.where(jnp.abs(w) < 0.1e-6, 0.0, w)

    vector = jnp.sin(angle_rad / 2) * unit_axis

    return jnp.concatenate([jnp.asarray(w).reshape(1), vector])


def q_to_angle_axis(quat: jnp.ndarray, degrees=False):
    w, x, y, z = quat

    angle = 2 * jnp.arccos(w)

    if abs(angle) < 1e-6:
        return 0, jnp.zeros(3)
    ...
    return angle, jnp.array([i, j, k])


# ----- Quaternion and Rotation Matrix -----#
def q_to_DCM(q: jnp.ndarray):
...
def DCM_to_q(DCM: jnp.ndarray):
    """
    https://motoq.github.io/doc/tnotes/dcm
    """

    assert jnp.shape(DCM) == (3, 3)
    ...
    if tr > c11 and tr > c22 and tr > c33:
        w = jnp.sqrt((1 + c11 + c22 + c33) / 4)
        x = (c23 - c32) / 4 / w
        y = (c31 - c13) / 4 / w
        z = (c12 - c21) / 4 / w
    elif c11 > c22 and c11 > c33:
        x = jnp.sqrt((1 + c11 - c22 - c33) / 4)
        w = (c23 - c32) / 4 / x
        y = (c12 + c21) / 4 / x
        z = (c31 + c13) / 4 / x
    elif c22 > c33:
        y = jnp.sqrt((1 - c11 + c22 - c33) / 4)
        w = (c31 - c13) / 4 / y
        x = (c12 + c21) / 4 / y
        z = (c23 + c32) / 4 / y
    else:
        z = jnp.sqrt((1 - c11 - c22 + c33) / 4)
        w = (c12 - c21) / 4 / z
        x = (c31 + c13) / 4 / z
        y = (c23 + c32) / 4 / z
...
def quat_apply(quat: jnp.ndarray, v: jnp.ndarray, passive=True):
    ...
    quat = unit(quat)
    v_quat = jnp.concatenate([jnp.zeros(1), jnp.asarray(v)])

    result = jnp.where(
        passive,
        mul(mul(conj(quat), v_quat), quat),
        mul(mul(quat, v_quat), conj(quat)),
    )
